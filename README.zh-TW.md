...
預覽站使用的是經過壓縮的 GIF 素材，適合線上瀏覽和快速搜尋；Docker/本機完整體驗請下載 `gifs-vault` 素材包。

## 演示

### 搜尋演示

![搜尋演示](Search_demo.gif)

### 合集瀏覽演示

![合集瀏覽演示](Collections_demo.gif)

## GIF 素材包下載

`gifs-vault` 素材包包含 Docker 和本機服務預覽所需的 GIF 檔案。可以從以下任一來源下載：
...
- [夸克網盤分流](https://pan.quark.cn/s/83a6fbba44c6?pwd=xdU9)（提取碼：`xdU9`）

下載後建議使用 release 中的 `capoo-vault-gifs-vault-20260629.sha256` 校驗分卷檔案完整性。解壓後將 `gifs-vault` 資料夾放到專案根目錄下。
...
# 從 GitHub Releases、Google Drive 或夸克網盤分流下載所有 capoo-vault-gifs-vault-YYYYMMDD.7z.00* 分卷檔案
...
- 部分 GIF 在瀏覽器預覽時可能出現背景閃爍，通常與原始貼紙素材、透明背景處理或 GIF 最佳化過程有關。
- 資料集整合自多個貼紙包來源，部分表情包可能存在重複或近似重複。
- TF-IDF 是本機關鍵字比對，適合明確短詞，但對模糊語義、複雜情緒或同義表達不一定準確；部分貼紙標註也可能存在偏差。
- 如果搜尋結果不理想，建議優先使用 Collections/合集瀏覽功能，按貼紙包逐組查找通常更穩定。
