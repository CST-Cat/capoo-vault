...
下载后建议使用 release 中的 `capoo-vault-gifs-vault-20260629.sha256` 校验分卷文件完整性。
...
- TF-IDF 是本地关键词匹配，适合明确短词，但对模糊语义、复杂情绪或同义表达不一定准确；部分贴纸标注也可能存在偏差。如果搜索结果不理想，更推荐先使用 Collections/合集浏览功能，按贴纸包直接翻找通常更稳定。
